...
from disasm import V4Instr, V4_VERBS
from resources import dump_all

def find_leave_room(instr_list: list[tuple[int, V4Instr]]):
    result = []
    for off, x in instr_list:
        if x.name == "loadRoomWithEgo":
            result.append({"offset": off, "room": x.args["room"], "op": "loadRoomWithEgo"})
        elif x.name == "putActorInRoom" and x.args["act"] == "VAR_EGO":
            result.append({"offset": off, "room": x.args["room"], "op": "putActorInRoom"})
    return result
        ...
        for obj_id, obj in room['objects'].items():
            for verb_id, verb in obj['verbs'].items():
                for match in find_leave_room(verb):
                    print({"room_src": room_id, "room_dest": match['room'], "type": "object", "obj_id": obj_id, "verb_id": verb_id})
